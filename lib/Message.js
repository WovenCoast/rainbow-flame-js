...
/**
 * @property {EnhancedGuild} guild The custom guild structure that handles this message
 * @property {EnhancedUser} author The custom user structure that sent this message
 */
class EnhancedMessage extends Message {
  ...
  async _processMessage(client) {
    ...
    if (
      !prefixes.some((p) =>
        this.content.toLowerCase().startsWith(p.toLowerCase())
      )
    )
      return;
    const prefix = prefixes
      .find((p) => this.content.toLowerCase().startsWith(p.toLowerCase()))
      .toLowerCase();
    const invoke = this.content
      ...
      .toLowerCase();
    if (
      !client.commands.has(invoke) &&
      !client.aliases.has(invoke) &&
      (await this.guild.db.get("invalidCommandMsg"))
    )
      return this.channel.send(
      ...
      .catch((err) => {
        ...
        this.channel.send(
          new MessageEmbed()
            .setTimestamp()
            .setAuthor(
              `${this.author.tag} | ${client.util.titleCase(command.name)}`,
              this.author.displayAvatarURL({ dynamic: true })
            )
            .setColor(client.colors.error)
            .setDescription(
              `An error occured. Are you sure your usage matches \`${command.usage.replace(
                "{prefix}",
                client.prefix[0]
              )}\`?\n\n${err}`
            )
...
module.exports = Structures.extend("Message", (Message) => EnhancedMessage);
