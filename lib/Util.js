...
const CustomGuild = require("./Guild");
...
class Util {
  ...
  /**
   * Find the member using their id, username or tag
   * @param {CustomGuild} guild The custom guild structure
   * @param {string} value The user's id, username or tag
   */
  parseMember(guild, value) {
    return (
      guild.members.cache.find(
        (m) => m.user.id === value.replace(/[^0-9]/gi, "")
      ) ||
      guild.members.fetch(value) ||
      guild.members.cache.find(
        (m) => m.nickname === value.replace(/[^A-Za-z0-9]/gi, "")
      ) ||
      guild.members.cache.find(
        (m) =>
          m.user.tag.replace(/[^A-Za-z0-9]/gi, "") ===
          value.replace(/[^A-Za-z0-9]/gi, "")
      )
    );
  }
  ...
  /**
   * Sanitize the result of an evaluation
   * @param {string} string The string to sanitize important env values from
   */
  sanitize(string) {
    return string.replace(
      new RegExp(
        `${Object.keys(process.env)
          .filter((e) =>
            ["TOKEN", "SECRET", "TRANSLATE"].some((s) => e.endsWith(s))
          )
          .map((k) => process.env[k])
          .join("|")}`,
        "gi"
  ...
  /**
   * Delay the actions after the promise has resolved
