...
class Util {
	...
	convertDuration(duration) {
		...
		const minutes = showWith0(Math.floor(duration / 60));
 ...
 */
	parseSongName(song) {
		return (song.title.includes(author) ? song.title.replace(song.author, `**${song.author}**`) : `**${song.title}** by *${song.author}*`) + `:${this.convertDuration(song.duration)}, requested by **${song.requestedBy}**`;
	}
}
