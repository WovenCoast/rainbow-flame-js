...
class Util {
	 ...
	 */
	parseSongName(song) {
		return (song.title.includes(song.author) ? song.title.replace(song.author, `**${song.author}**`) : `**${song.title}** by *${song.author}*`) + `:${this.convertDuration(song.duration)}, requested by **${song.requestedBy}**`;
	}
}
