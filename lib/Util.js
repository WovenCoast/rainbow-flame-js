...
class Util {
	...
	convertDuration(duration) {
		...
		const hours = showWith0(Math.floor((duration / 60) / 60));
		const days = showWith0(Math.floor(((duration / 60) / 60) / 24));
		return `${days !== "00" ? days + ":" : ""}${hours !== "00" ? hours + ":" : ""}${minutes}:${seconds}`;
	}
	/**
	 ...
	 */
	parseSongName(song) {
		return (song.title.includes(song.author) ? song.title.replace(song.author, `**${song.author}**`) : `**${song.title}** by *${song.author}*`) + ` requested by **${song.requestedBy}** : ${this.convertDuration(song.duration)}`;
	}
}
