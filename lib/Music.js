...
class Music {
	...
	async startPlaying(song, textChannel, voiceChannel) {
		...
		this.playing = true;
		this.musicUpdatesChannel.send(`:arrow_forward: Playing ${client.util.parseSongName(song)}`);
		this.loop = "noloop";
	...
	addSong(song) {
		if (this.playing) {
			...
			this.musicUpdatesChannel.send(`:white_check_mark: Successfully added ${client.util.parseSongName(song)}`)
		}
	}
	...
	async _finish() {
		...
		this.musicUpdatesChannel.send(`:arrow_forward: Playing ${client.util.parseSongName(this.songs[0])}`);
	}
