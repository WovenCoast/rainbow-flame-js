 ...
 * @property {Array<string>} owners The owners of this bot (used in owner only commands)
 ...
 * @property {any} playlists The playlists for the radio command
...
class Client extends Discord.Client {
	...
	constructor(opts, discordOpts) {
		...
		/**
		 * The owners of this bot (used in owner only commands)
		 * @type {Array<string>}
		 */
		...
		/**
		 * The playlists for the radio command
		 * @type {any}
		 */
		...
		this.db = {
			...
			client: new DBClient(this, this.schemas.client, { uri: 'sqlite://.data/client.sqlite', namespace: "client" })
		};
		/**
	...
	/**
	 * Parse the playlists of the radio command
	 * @returns {void}
	 */
	async _parsePlaylists() {
		Object.keys(this.playlists).forEach(playlistKey => {
			const playlist = this.playlists[playlistKey];
			playlist.forEach(async (song, index) => {
				this.playlists[playlistKey][index] = (await this._searchSongs(song))[0];
				if (index === playlist.length - 1) {
					Object.keys(this.playlists).forEach(playlistKey => {
						this.playlists[playlistKey] = this.playlists[playlistKey].filter(e => e !== undefined);
					});
				};
			});
		});
	...
	async _searchSongs(query) {
		...
		return res.videos.map(s => {
			...
			return { url: s.url, duration: s.seconds, author: s.author.name, title: s.title, requestedBy: undefined };
