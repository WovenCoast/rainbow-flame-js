...
class Client extends Discord.Client {
	constructor(opts, discordOpts) {
		...
		this.db = {};
		this.db.guild = new DB(this, schemas.guild, { uri: 'sqlite://.data/guilds.sqlite', namespace: "guild" });
		this.db.user = new DB(this, schemas.user, { uri: 'sqlite://.data/users.sqlite', namespace: "user" });
		this.db.member = new DB(this, schemas.member, { uri: 'sqlite://.data/members.sqlite', namespace: "member" });
		this.categories = {};
	...
	_readCommands(folder) {
		fs.readdirSync(path.join(require.main.path, folder)).forEach(dir => {
			this.categories[this.util.titleCase(dir)] = [];
			fs.readdirSync(path.join(require.main.path, folder, dir)).forEach(
				commandPath => {
					...
					command.category = this.util.titleCase(dir);
