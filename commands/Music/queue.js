...
module.exports = {
  ...
  async exec(client, message, args) {
    ...
    return message.channel.send(new Discord.MessageEmbed().setTimestamp().setColor(client.colors.info).setAuthor(`${message.author.tag} | Queue`, message.author.displayAvatarURL()).setTitle(`Now playing: ${client.util.parseSongName(message.guild.music.songs[0])}, playhead at ${client.util.convertDuration(message.guild.music.duration)}`).setDescription(songs.map((song, index) => `**${((page - 1) * 5) + (index + 1)}**: ${client.util.parseSongName(song)}`)).setFooter(`Page ${page} of ${Math.ceil(message.guild.music.songs.length / 5)}`));
  }
}