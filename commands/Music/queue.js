...
module.exports = {
  ...
  async exec(client, message, args) {
    ...
    return message.channel.send(new Discord.MessageEmbed().setTimestamp().setColor(client.colors.info).setAuthor(`${message.author.tag} | Queue`, message.author.displayAvatarURL()).setTitle(`Now playing: ${parseSongName(message.guild.music.songs[0].title, message.guild.music.songs[0].author)}`).setDescription(songs.map((song, index) => `**${((page - 1) * 5) + (index + 1)}**: ${parseSongName(song.title, song.author)}`)).setFooter(`Page ${page} of ${Math.ceil(message.guild.music.songs.length / 5)}`));
  }
}
