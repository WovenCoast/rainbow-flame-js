module.exports = {
	...
	async exec(client, message, args) {
		...
		const songIndex = isNaN(args[0]) ? message.guild.music.songs.map(s => s.title.toLowerCase()).indexOf(args.join(" ").toLowerCase()) : parseInt(args[0]) - 1;
		if (songIndex <= 0 || songIndex >= message.guild.music.songs.length) throw new Error("You cannot a non existent song!");
		const removedSong = message.guild.music.songs[songIndex];
		message.guild.music.removeSong(songIndex);
		return message.channel.send(`:white_check_mark: Successfully removed ${client.util.parseSongName(removedSong)}`);
	}
}