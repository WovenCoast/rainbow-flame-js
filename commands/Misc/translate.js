...
const translate = require("yandex-translate")(process.env.YANDEX_TRANSLATE)
  .translate;
  ...
  async exec(client, message, args) {
    ...
    if (lang == "help") {
      ...
      if (embeds[0] == undefined) {
        ...
        const msg = await message.channel.send(
          embeds[current].setFooter(`Page ${current + 1} of ${embeds.length}`)
        );
        await msg.react("◀");
        await msg.react("▶");
    ...
    translate(
      ...
      (err, res) => {
        ...
        if (res.code !== 200)
          throw new Error(
            `${lang} is not a valid language! To view the list of valid languages try \`${client.prefix[0]}translate help\``
          );
        const embed = new Discord.MessageEmbed()
          .setColor(client.colors.success)
          .setAuthor(
            `${message.author.tag} | ${res.lang} translation`,
